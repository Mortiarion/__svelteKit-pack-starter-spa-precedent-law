...
<Section id='our_services_mobile' classes="xl:hidden">
	<Container classes="font-source font-semibold">
		...
		<Button
			...
			type="button"
			classes="flex flex-row-reverse gap-2.5 justify-center items-center text-sm font-source font-semibold mb-10"
		>
			{activeCategory}
			<svg
				width="20"
		...
		{#if showFilter}
			<Fade_In_Out duration={1000} threshold={0.2}>
				<div class="filter-buttons">
					...
					{/each}
					{#if subCategories[activeCategory] && subCategories[activeCategory].length}
						<div class="subcategories">
							{#each subCategories[activeCategory] as subCat}
								<button
									class={`filter-button ${subCat === subCategory ? 'active' : ''}`}
									on:click={() => setSubCategory(subCat)}
								>
									{subCat}
								</button>
							{/each}
